...
async def get_id_available_terminals(
    shop_id: int,
    ...
    if not terminals:
        raise HTTPException(
            status_code=404, detail="No terminals found for this shop")
...
async def authorize_terminal(
    ...
    if terminal.shop_id != shop_id:
        raise HTTPException(
            status_code=403,
            detail="Invalid shop ID, terminal not authorized for this shop",
        )
